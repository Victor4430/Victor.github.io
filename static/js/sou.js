$(document).ready(function() {
	...
	var quick_list_preinstall = {
		'1': {
			...
			img: "./static/icon/blog.png",
			explain: "Victor's Blog",
		},
		'2': {
			title: "GitHub",
		...
		'12': {
			title: "传统文化",
			url: "https://cesuan.demo520.com/",
			img: "./static/icon/taijitu2.gif",
			explain: "传统文化",
		},
		'13': {
			title: "小游戏",
			url: "http://open14275.720sy.com/",
			img: "./static/icon/720.png",
			explain: "小游戏",
		},
		'14': {
			title: "装备变现",
			url: "http://www.5173.com/?recommenduserid=US23010884867192-0020",
			img: "./static/icon/jinbi.png",
			explain: "装备变现",
		},
	};
	...
	function seList() {
		...
		for (var i in se_list) {
			html +=
				...
				se_list[i]["url"] + "' name='" + se_list[i]["name"] + "' img='" + se_list[i]["img"] +
				"'><img src='" + se_list[i]["img"] + "'></img>" + se_list[i]["title"] + "</li>";
		}
	...
	function setSeInit() {
		...
		for (var i in se_list) {
			...
			tr += "<td>" + i + ". " + se_list[i]["title"] + "</td><td><button class='set_se_default' value='" +
				i + "'><span class='iconfont iconstrore-add'></span></button><button class='edit_se' value='" +
				i +
				"'><span class='iconfont iconbook-edit'></span></button> <button class='delete_se' value='" +
				i + "'><span class='iconfont icondelete'></span></button></td></tr>";
			html += tr;
		}
	...
	$(".set_se_list_preinstall").click(function() {
		var r = confirm(
			"Current settings will be removed! (You'd better backup before performing this)");
		if (r) {
			setSeList(se_list_preinstall);
	...
	$("#my_data_in").click(function() {
		...
		if (confirm("Current settings will be erased, continue?")) {
			...
			if (mydata["se_default"]) {
				Cookies.set('se_default', mydata["se_default"], {
					expires: 36500
				});
			}
